...
wt.on("format-tab-title", function(tab, _, _, config, hover, max_width)
  ...
  Title:append(bg, tab_fg, tabicons.left, attributes)
  Title:append(bg, fg, "", attributes)
  Title:append(tab_fg, bg, (unseen_output and Icon.Notification .. " " or ""), attributes)

  ---the formatted tab title
  Title:append(tab_fg, tab_font, title .. " ", attributes)
  Title:append(tab_circle_right, index_font, " " .. tostring(tab_idx), attributes)

  -- ---the right tab bar separator
  Title:append(bg, tab_circle_right, tabicons.right, attributes)
