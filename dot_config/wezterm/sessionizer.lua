...
local git = (file_exists("C:\\Program Files\\Git\\cmd\\git.exe") or file_exists("/usr/bin/git"))
		...
		act.InputSelector({
			action = wt.action_callback(function(window, pane, id, label)
				if not id and not label then
					...
					window:perform_action(act.SwitchToWorkspace({ name = id, spawn = { cwd = label } }), pane)
				end
			end),
			fuzzy = true,
